...
const Profil = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      setSelectedAvatar(storedAvatar || avatar0);

      const storedUserName = localStorage.getItem('username');
      setUserName(storedUserName || user.displayName || 'Nom d\'utilisateur');

      setUserEmail(user.email);
      const creationTimestamp = user.metadata.creationTime;
      setCreationDate(new Date(creationTimestamp).toLocaleDateString('fr-FR'));
    }
  }, []);
        ...
        <div className={style['profil-box']}>
          ...
          <div className={style['profile-container']}>
            <div className={style['profile-header']}>
              <img id="avatar" src={selectedAvatar} alt="Avatar" />
